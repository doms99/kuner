...
final Animatable<double> _initialScaleTransition = Tween<double>(
  begin: 0.6,
  end: 1,
);

final Animatable<double> _initialOpacityTransition = Tween<double>(
  begin: 0,
  end: 1,
);
...
///  * [WearOsPageRoute], which is a [PageRoute] that leverages this mixin.
mixin WearOsRouteTransitionMixin<T> on PageRoute<T> {
  ...
  /// The title string of the previous [WearOsPageRoute].
  ///
  ...
  void didChangePrevious(Route<dynamic>? previousRoute) {
    final String? previousTitleString = previousRoute is WearOsRouteTransitionMixin ? previousRoute.title : null;
  ...
  bool canTransitionTo(TransitionRoute<dynamic> nextRoute) {
    ...
    return nextRoute is WearOsRouteTransitionMixin && !nextRoute.fullscreenDialog;
  }
  ...
  /// screen dialog, otherwise a [WearOsPageTransition] is returned.
  ///
  /// Used by [WearOsPageRoute.buildTransitions].
  ///
  /// This method can be applied to any [PageRoute], not just
  /// [WearOsPageRoute]. It's typically used to provide a Cupertino style
  ...
  static Widget buildPageTransitions<T>(
    ...
    final backGestureInProgress = isPopGestureInProgress(route);

    return WearOsPageTransition(
      backGestureInProgress: backGestureInProgress,
      primaryRouteAnimation: animation,
      secondaryRouteAnimation: secondaryAnimation,
      linearTransition: linearTransition,
      child: _WearOsBackGestureDetector<T>(
...
/// See also:
///
///  * [WearOsRouteTransitionMixin], for a mixin that provides iOS transition
...
///  * [WearOsPage], for a [Page] version of this class.
class WearOsPageRoute<T> extends PageRoute<T> with WearOsRouteTransitionMixin<T> {
  ...
  @override
  Widget buildContent(BuildContext context) => _clippedBuilder(context);
...
class _PageBasedCupertinoPageRoute<T> extends PageRoute<T> with WearOsRouteTransitionMixin<T> {
  _PageBasedCupertinoPageRoute({
    required WearOsPage<T> page,
  })  : assert(page != null),
        super(settings: page) {
    assert(opaque);
  }

  WearOsPage<T> get _page => settings as WearOsPage<T>;
...
/// See also:
///
///  * [WearOsPageRoute], for a [PageRoute] version of this class.
class WearOsPage<T> extends Page<T> {
...
class WearOsPageTransition extends StatelessWidget {
  ...
  WearOsPageTransition({
    super.key,
    required this.backGestureInProgress,
  ...
  final bool backGestureInProgress;
...
class _WearOsBackGestureDetector<T> extends StatefulWidget {
  const _WearOsBackGestureDetector({
    super.key,
  ...
  @override
  _WearOsBackGestureDetectorState<T> createState() => _WearOsBackGestureDetectorState<T>();
}

class _WearOsBackGestureDetectorState<T> extends State<_WearOsBackGestureDetector<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    // that has the notch.
    double dragAreaWidth = MediaQuery.of(context).size.width;
...
///
/// This is created by a [WearOsPageRoute] in response from a gesture caught
/// by a [_WearOsBackGestureDetector] widget, which then also feeds it input
