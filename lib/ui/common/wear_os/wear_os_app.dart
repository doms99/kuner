...
/// [WearOsApp] uses this [TextStyle] as its [DefaultTextStyle] to encourage
...
/// Describes which theme will be used by [WearOsApp].
enum ThemeMode {
...
/// [GridPaper].
///
/// [WearOsApp] configures its [WidgetsApp.textStyle] with an ugly red/yellow
...
///
/// The [WearOsApp] configures the top-level [Navigator] to search for routes
...
/// The [WearOsApp] automatically creates a [DefaultSelectionStyle]. It uses
/// the colors in the [ThemeData.textSelectionTheme] if they are not null;
/// otherwise, the [WearOsApp] sets [DefaultSelectionStyle.selectionColor] to
...
///
/// {@tool snippet}
/// This example shows how to create a [WearOsApp] that disables the "debug"
...
///
/// {@tool snippet}
/// This example shows how to create a [WearOsApp] that uses the [routes]
...
///
/// {@tool snippet}
/// This example shows how to create a [WearOsApp] that defines a [theme] that
...
class WearOsApp extends StatefulWidget {
  ...
  /// Creates a [WearOsApp] that uses the [Router] instead of a [Navigator].
  ///
  /// {@macro flutter.widgets.WidgetsApp.router}
  const WearOsApp.router({
    super.key,
    this.scaffoldMessengerKey,
  ...
  /// ```
  ///
  /// Constructing a [WearOsApp] with a `FooLocalizationsDelegate` overrides
  ...
  @override
  State<WearOsApp> createState() => _WearOsAppState();

  /// The [HeroController] used for Material page transitions.
  ///
  /// Used by the [WearOsApp].
...
/// Describes how [Scrollable] widgets behave for [WearOsApp]s.
///
...
class _WearOsAppState extends State<WearOsApp> {
  ...
  void initState() {
    super.initState();
    _heroController = WearOsApp.createMaterialHeroController();
  }
  ...
  Widget _buildWidgetApp(BuildContext context) {
    ...
    return WidgetsApp(
      ...
      pageRouteBuilder: <T>(RouteSettings settings, WidgetBuilder builder) {
        return WearOsPageRoute<T>(settings: settings, builder: builder);
      },
      home: widget.home,
